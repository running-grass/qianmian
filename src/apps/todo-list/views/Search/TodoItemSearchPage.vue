...
async function doSearch() {
  ...
  const [list] = await db.query<[TodoItem[]]>(`SELECT * FROM todo_item WHERE title CONTAINS $keyword`, {
...
<template>
    ...
    <section class="flex-1 flex flex-col p-4">
      ...
      <ul v-else>
        <TodoItemRow v-for="todoItem of todoItemList" :key="todoItem.entity_id.id.toString()" :todoItem="todoItem"
          showList @click="selectedTodoItem = todoItem; mobileDrawer = true" :class="[
  ...
  <el-drawer v-if="isMobileScreen" v-model="mobileDrawer" size="90%" :with-header="false" destroy-on-close
    direction="btt">
