...
const PomodoroStatusValidator = z.enum(['inactive', 'pomodoro', 'finish', 'short_break', 'long_break', 'finish_break'])
const CurrentPomodoroValidator = z.object({
  ...
  /** 开始时间 */
  start_time: z.number().positive().default(() => myDayjs().unix()),
  break_start_time: z.number().positive().default(() => myDayjs().unix()),
})

type CurrentPomodoro = z.infer<typeof CurrentPomodoroValidator>

const currentPomodoro = useStorage<CurrentPomodoro>('currentPomodoro', () => CurrentPomodoroValidator.parse({}));
...
async function finishPomodoro() {
  ...
  await createEntityEventLog(pomodoroEventSlug, { pomodoro_time: currentPomodoro.value.pomodoro_time })
}


// 休息部分
const breakEndTime = computed(() => {
  return myDayjs(currentPomodoro.value.break_start_time * 1000).add(currentPomodoro.value.break_time, 'minute')
})
...
if (currentPomodoro.value.status === 'pomodoro') {
  resumePomodoroInterval();
} else if (currentPomodoro.value.status === 'short_break' || currentPomodoro.value.status === 'long_break') {
  resumeBreakInterval();
}
...
<template>
  <section class="w-full h-full flex flex-col items-center justify-center">
    ...
    <section class="relative">
      <Knob :model-value="process" readonly :showValue="false" rangeColor="MediumTurquoise" valueColor="SlateGray"
        :size="400"></Knob>
      <span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-2xl font-bold">{{ timeText }}</span>
    </section>
    <Button v-if="['inactive', 'finish_break'].includes(currentPomodoro.status)" label="开始番茄" class="mt-4"
      @click="startPomodoro"></Button>

    <Button v-if="currentPomodoro.status === 'finish' && nextBreakType === 'short_break'" label="开始短休息" class="mt-4"
      @click="startShortBreak"></Button>
    <Button v-else-if="currentPomodoro.status === 'finish' && nextBreakType === 'long_break'" label="开始长休息" class="mt-4"
      @click="startLongBreak"></Button>

    <Button v-if="currentPomodoro.status !== 'inactive'" label="退出番茄" class="mt-4" @click="exitPomodoro"
